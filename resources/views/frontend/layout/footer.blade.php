            ...
            <div class="latest-posts">
              <?php
               $currentDate = date('Y-m-d');
                $eventdata=DB::table('events')->select('date',DB::raw('MONTHNAME(date) AS MONTHNAME'), 'name', 'description')->whereDate('date','>=',$currentDate)->orderBy('date','ASC')->limit(3)->get();
                foreach($eventdata as $post){
              ?>
